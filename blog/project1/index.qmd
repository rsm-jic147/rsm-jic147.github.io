...
# Drop missing values
df_clean = df[['mrm2', 'treatment']].dropna()

# Separate treatment and control groups
treat = df_clean[df_clean['treatment'] == 1]['mrm2']
control = df_clean[df_clean['treatment'] == 0]['mrm2']
...
# T-statistic (manual, class formula)
se = np.sqrt(var1 / n1 + var0 / n0)
t_stat = (mean1 - mean0) / se
df_deg = min(n1 - 1, n0 - 1)  # degrees of freedom
...
```
Both the manual t-test and the linear regression show that there is no statistically significant difference in mrm2 (months since last donation) between the treatment and control groups:

- Treatment group mean: 13.012
- Control group mean: 12.998
- t-statistic: 0.12
- p-value: 0.905

In the regression, the estimated coefficient on treatment is 0.0137 with a p-value of 0.905, confirming the same result: no significant difference.

This matches the findings in Table 1 of Karlan & List (2007), which reports that mrm2 (listed as Number of months since last donation) has nearly identical means between treatment (13.012) and control (12.998) groups.

Table 1 in the paper is significant as it shows randomization worked.Random assignment should ensure that treatment and control groups are statistically equivalent in expectation on all pre-treatment characteristics. By showing that variables like mrm2, freq, female, etc. are not significantly different, the authors give readers confidence that:

- There’s no selection bias
- Any differences in donation outcomes (like gave or amount) can be causally attributed to the treatment
...
```{python}
# regression
# Filter treatment group and create dummy variables
df_ratio_reg = df[(df['treatment'] == 1) & (df['gave'].notnull()) & (df['ratio'].isin([1, 2, 3]))].copy()

df_ratio_reg['ratio1'] = (df_ratio_reg['ratio'] == 1).astype(int)
df_ratio_reg['ratio2'] = (df_ratio_reg['ratio'] == 2).astype(int)
df_ratio_reg['ratio3'] = (df_ratio_reg['ratio'] == 3).astype(int)

# Regression: gave ~ ratio2 + ratio3 (baseline is 1:1)
import statsmodels.formula.api as smf
model = smf.ols('gave ~ ratio2 + ratio3', data=df_ratio_reg).fit()
print(model.summary())
```

To assess whether larger match ratios increase the likelihood of donating, I ran a linear probability regression of gave on dummy variables for the 2:1 and 3:1 match ratios (with 1:1 as the baseline). The results show small positive coefficients for both 2:1 (0.0019) and 3:1 (0.0020) match rates, but neither is statistically significant (p > 0.3). This indicates that while higher match ratios slightly increase donation rates, the differences are not meaningful. These findings support the authors’ interpretation that increasing the match ratio beyond 1:1 does not significantly affect the probability of donating.

```{python}
# data vs. regression

# Step 1: Compute response rate differences directly from the data
direct_diff_21_vs_11 = gave_2to1.mean() - gave_1to1.mean()
direct_diff_31_vs_21 = gave_3to1.mean() - gave_2to1.mean()

# Step 2: Compute the differences in coefficients from regression
# Since ratio1 (1:1) is the baseline, it has a coefficient of 0
# So: ratio2 - ratio1 = coef_ratio2, ratio3 - ratio2 = coef_ratio3 - coef_ratio2
reg_diff_21_vs_11 = model.params['ratio2']
reg_diff_31_vs_21 = model.params['ratio3'] - model.params['ratio2']

# Format results
{
    "Direct Difference (2:1 vs 1:1)": direct_diff_21_vs_11,
    "Direct Difference (3:1 vs 2:1)": direct_diff_31_vs_21,
    "Regression Coeff. Diff (2:1 vs 1:1)": reg_diff_21_vs_11,
    "Regression Coeff. Diff (3:1 vs 2:1)": reg_diff_31_vs_21
}
